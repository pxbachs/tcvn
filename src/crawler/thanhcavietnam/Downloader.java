	...
	public void run() {
		...
		try {
			...
			if (target instanceof File) {
				...
				try {
					...
					if (f.isFile()) {
						...
						if (!f.exists())
							f.mkdirs();
						filename = getFileName(link.getURL());
						
						File tf = new File (f.getAbsolutePath().concat(File.separator).concat(filename));
						if(tf.exists() && (tf.length() == this.totalLength || !filename.endsWith(".mp3"))){
							 needDownload = false;
						} else {
			...
			if(needDownload){
				...
				try {
					if (target instanceof File) {
						//ThanhCaVietNam.getLogger().info("Downloading " + ((File)target).getAbsolutePath());
				...
				/* download completed successfully */
				progressString = "Download completed [File existed!]";
				progressUpdated = true;
			}
		} catch (Exception e) {
			error = e;
