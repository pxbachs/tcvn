...
public class LightXmlParser {
	...
	public static void parse(InputStream inputStream, String encoding, LightXmlParserHandler handler) throws IOException {
		...
		// Check processing instructions (only if no tag was read before) to
		// extract encoding
		boolean invalidHeader = false;
		if (c == 239) {
			System.out.println("Start with BOM UTF-8\t" + ((int) '<'));
			System.out.println(c = inputStream.read());
			System.out.println(c = inputStream.read());
			System.out.println(c = inputStream.read());
			// System.out.println(c = inputStream.read());
		}
			...
			while (c != -1) {
				if (c == '<' || tagOpen) {
					...
					if (c == '/') { // endtag
						...
						for (int i = 0; i < tagName.length(); i++) { // compare
							// open
							// tag
							// and
							// close
	...
	public static Element parse(InputStream inputStream, String encoding) throws IOException {
		final Element root = (new Element()).createElement("root");
		parse(inputStream, encoding, new LightXmlParserHandler() {
			Element currentParent;

			Element currentElement;

			public void characters(String characters, boolean isCDATA) {
				currentElement.addChild(Node.TEXT, characters);
			}
			...
			public void endElement(String name) {
				currentParent = (Element) currentParent.getParent();
			}
			...
			public void startElement(String name, Hashtable attributes) {
				currentElement = currentParent.createElement(name);
				currentElement.setParent(currentParent);
				currentParent.addChild(Node.ELEMENT, currentElement);
				if (attributes != null) {
					Enumeration key = attributes.keys();
					if (key != null)
						while (key.hasMoreElements()) {
							Object k = key.nextElement();
							currentElement.setAttribute(k.toString(), attributes.get(k).toString());
						}
				}
